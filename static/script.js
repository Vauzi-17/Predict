...
function setActiveTool(tool) {
    document.querySelectorAll("[data-tool-target]").forEach(btn => {
        btn.classList.toggle("active", btn.dataset.toolTarget === tool);
    });
...
function initToolSwitch() {
    document.querySelectorAll("[data-tool-target]").forEach(btn => {
        btn.addEventListener("click", () => setActiveTool(btn.dataset.toolTarget));
    });
    setActiveTool(document.body.dataset.activeTool || "transaction");
}
...
function validateTransaction(formData) {
    ...
    [
        ["country",           "err-country",     "Negara User"],
...
function showResult(tool, key) {
    ...
    card.className = `result-card ${data.kelas} mx-5 mb-5 rounded-2xl border border-neutral-200 p-6 lg:mx-8 lg:mb-8`;
    iconWrap.innerHTML = "";
    iconWrap.appendChild(makeLucideIcon(data.iconName));
    eyebrowEl.textContent = data.eyebrow;
    statusEl.textContent  = key;
    keteEl.textContent    = data.keterangan;

    saranEl.innerHTML = "";
    // FIX: gunakan icon yang sesuai per kelas
    const saranIcon = (data.kelas === "fraud" || data.kelas === "sangat-pendek")
        ? "triangle-alert"
        : (data.kelas === "pendek" ? "circle-alert" : "circle-check");

    data.saran.forEach(text => {
        const li   = document.createElement("li");
        li.appendChild(makeLucideIcon(saranIcon));
